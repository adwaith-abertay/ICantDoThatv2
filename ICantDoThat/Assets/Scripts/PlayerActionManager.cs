...
public class PlayerActionManager : MonoBehaviour
{
    ...
    public void GenerateEnergy()
    {
        ...
        currentEnergy = Mathf.Min(currentEnergy + gained, 15); // cap at 15
    ...
    public void ActionStartFire()
    {
        ...
        CharacterMovement target = GetCrewClosestToSwitch(crew);
        if (target != null)
        {
            FireSpread.Instance.IgniteTileByName(target.GetCurrentTile());
            currentEnergy -= 10;
            Debug.Log($"Fire started on {target.GetCurrentTile()}. Energy left: {currentEnergy}");
        }
